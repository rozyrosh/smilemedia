...
type DomeGalleryProps = {
  ...
  grayscale?: boolean;
  className?: string;
  /** Slowly spin the dome when idle. Default `true`. */
  autoRotate?: boolean;
  /** Horizontal spin speed in degrees per second. Default `5`. */
...
function buildItems(pool: DomeGalleryImage[], seg: number): TileCoord[] {
  ...
  const usedImages = Array.from(
    { length: totalSlots },
    (_, i) => normalizedImages[i % normalizedImages.length] ?? { src: "", alt: "" },
  );
...
export function DomeGallery({
  ...
  const isAutoRotatePaused = useCallback(() => {
    return (
      draggingRef.current ||
      Boolean(focusedElRef.current) ||
      openingRef.current ||
      inertiaRAF.current !== null ||
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      if (!isAutoRotatePaused()) {
        if (lastAutoTickRef.current == null) {
          lastAutoTickRef.current = timestamp;
        } else {
          const dt = (timestamp - lastAutoTickRef.current) / 1000;
          const nextY = wrapAngleSigned(
            rotationRef.current.y + autoRotateSpeed * dt,
          );
  ...
  }, [autoRotate, autoRotateSpeed, isAutoRotatePaused, stopAutoRotate]);
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalImg) {
        ...
        img.style.cssText = "width:100%;height:100%;object-fit:contain;background:#122040;";
  ...
  return (
    <div
      ref={rootRef}
      className={["sphere-root", className].filter(Boolean).join(" ")}
      style={
        {
          "--segments-x": segments,
          "--segments-y": segments,
